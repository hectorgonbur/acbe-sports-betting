...
if menu == "🎓 Guía Interactiva":
    ...
    if modulo == "🏠 Introducción":
        ...
        with col2:
            st.image("https://via.placeholder.com/300x400/2c3e50/ffffff?text=Sistema+ACBE", 
                    caption="Arquitectura del Sistema", use_container_width=True)
    ...
    **Recuerda:** El éxito viene de la **consistencia** y **gestión de riesgo**, no de adivinar resultados.
    ...
    def parse_percentage(value):
        """Convierte '45%' a 0.45, o deja float igual"""
        if isinstance(value, str) and '%' in value:
            return float(value.replace('%', '').strip()) / 100
        return float(value)

    def format_percentage(value):
        """Convierte 0.45 a '45.0%'"""
    ...
    if 'probabilidades_numericas' not in st.session_state:
        st.session_state['probabilidades_numericas'] = {}
        
    if 'picks_con_valor' not in st.session_state:
        st.session_state['picks_con_valor'] = []
    ...
    class SistemaLogging:
        ...
        def registrar_pick(self, pick_data):
            ...
            if pick_data.get('ev', 0) > 0:
                self.performance['picks_ev_positivo'] += 1

    # Inicializar logger en session_state
        ...
        def generar_escenarios(prob, cuota, bankroll_inicial=1000, n_apuestas=100, n_simulaciones=5000):
            ...
            for sim in range(n_simulaciones):
                ...
                for apuesta in range(n_apuestas):
                    ...
                    drawdown_actual = (peak - bankroll) / peak if peak > 0 else 0
    ...
    class RecomendadorInteligente:
        ...
        def generar_recomendacion(self, analisis):
            ...
            mejor_pick = self._encontrar_mejor_pick(analisis.get('resultados', []))
        ...
        def _encontrar_mejor_pick(self, resultados):
            ...
            for r in resultados:
                try:
                    ev = parse_percentage(r.get('EV', 0))
                    if ev > 0.02:  # Solo picks con EV > 2%
                        picks_con_ev.append({
                            'Resultado': r['Resultado'],
                            'EV': ev,
                            'Cuota Mercado': float(r['Cuota Mercado']),
                            'Prob Modelo': parse_percentage(r.get('Prob Modelo', 0)),
        ...
        def _generar_razones(self, pick, analisis):
            ...
            cuota_justa = 1 / pick['Prob Modelo'] if pick['Prob Modelo'] > 0 else 999
        ...
        def exportar_resumen_html(recomendacion, resultados):
            ...
            <head>
                ...
                <style>
                    ...
                    .recomendacion {{ background: {'#2ecc71' if recomendacion.get('pick') else '#e74c3c'}; 
            ...
            <body>
                ...
                <div class="recomendacion">
                    <h2>🎯 {'RECOMENDACIÓN: ' + recomendacion['accion'] if recomendacion.get('pick') else 'NO RECOMENDACIÓN'}</h2>
            """
            ...
            for r in resultados:
                ev_val = parse_percentage(r.get('EV', 0))
                ev_class = "green" if ev_val > 0 else "red"
                html += f"""
                    <tr>
                        <td>{r['Resultado']}</td>
                        <td>{r['Prob Modelo']}</td>
    ...
    def ejecutar_fase_4(picks_con_valor, gestor_riesgo, backtester, bankroll=1000, 
        ...
        for r in picks_con_valor:
            try:
                ...
                prob_modelo_numerico = parse_percentage(r.get("Prob Modelo", 0))
                cuota_numerico = float(r.get("Cuota Mercado", 1.0))
                ev_numerico = parse_percentage(r.get("EV", 0))
                ...
                recomendaciones.append({
                    ...
                    "ev": format_percentage(ev_numerico),
        ...
        for rec in recomendaciones:
            ...
            if rec.get("kelly_pct", 0) > 0:
                with st.expander(
                    ...
                    if rec.get('backtest_metrics', {}).get('distribucion_retornos') is not None:
                        fig_dist = go.Figure()
        ...
        if recomendacion.get('pick'):
            ...
            if pick_data:
                ...
                with col_det1:
                    ...
                    st.metric("Cuota Justa", pick_data.get('Fair Odd', 'N/A'))
    ...
    if st.sidebar.button("🚀 EJECUTAR ANÁLISIS COMPLETO", type="primary", use_container_width=True, key="btn_ejecutar"):
        ...
        with st.spinner("🔍 DETECTANDO INEFICIENCIAS..."):
            ...
            for label, p_modelo, p_mercado, se, cuota in zip(
                ...
                resultado_dict = {
                    "Resultado": label,
                    "Prob Modelo": format_percentage(p_modelo),
                    "Prob Mercado": format_percentage(p_mercado),
                    "Delta": format_percentage(p_modelo - p_mercado),
                    "EV": format_percentage(ev),
                ...
                resultados_analisis.append(resultado_dict)
                
                # Agregar a picks con valor si cumple criterios
                if value_analysis['significativo'] and ev > 0.02:
                    picks_con_valor.append(resultado_dict)
        ...
        with st.spinner("📊 GENERANDO REPORTE FINAL..."):
            ...
            if recomendaciones:
                ev_promedio = np.mean([parse_percentage(r['ev']) for r in recomendaciones])
            ...
            if picks_con_valor:
                for pick in picks_con_valor:
                    ev_val = parse_percentage(pick['EV'])
        ...
        # Mostrar módulo de recomendación
